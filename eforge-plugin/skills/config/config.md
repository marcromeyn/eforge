...
1. **Backend selection** (required) - Which LLM backend: `claude-sdk` (uses Claude Code's built-in SDK), `pi` (experimental, multi-provider via Pi SDK supporting OpenRouter, Anthropic, OpenAI, Google, etc.), or `codex` (uses OpenAI Codex SDK — requires the `codex` CLI to be installed).
